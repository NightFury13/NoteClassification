...
import sys
from aubio import source, pitch, freqtomidi, pvoc, mfcc
import pandas as pd
...
while True:
    ...
    if (confidence > tolerance) and (pitch!=0):
        spec = p(samples)
        mfcc_out = m(spec)
        mfcc_of_notes.append({'pitch':pitch, 'confidence':confidence, 'note_sample':samples, 'mfcc':mfcc_out, 'frame_idx':total_frames, 'time_idx':len(pitches)*hop_s}) 
...
#save the MFCC
mfcc_tosave = pd.DataFrame(mfcc_of_notes)
mfcc_tosave.to_pickle(filename+'_mfcc_feats.pkl')
